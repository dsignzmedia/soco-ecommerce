...
// Freeze panes is handled via XML configuration above
        ...
        return response($html, 200)
            ...
            ->header('Content-Disposition', 'attachment; filename="products_bts_export-' . date('Y-m-d') . '.xls"');
    }

    protected function downloadPdf($products)
    {
        $pdf = \Barryvdh\DomPDF\Facade\Pdf::loadView('admin.back_to_school.products.export-pdf', ['products' => $products]);
        return $pdf->download('products_bts_export-' . date('Y-m-d') . '.pdf');
    }
