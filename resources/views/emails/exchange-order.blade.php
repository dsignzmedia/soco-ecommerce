...
<title>Exchange Order Generated - The Skool Store</title>
...
Exchange #{{ $exchangeOrder->order_number }}
</td>
</tr>
</table>
</td>
...
Exchange Order Generated!
</h1>

<p style="margin:0 0 20px;color:#9a6c4c;">
Great news, {{ $exchangeOrder->customer_name }}! We've created your exchange order. Your replacement item will be shipped to you soon.
</p>
...
#{{ $exchangeOrder->order_number }}
</p>

<p style="margin:0;font-size:13px;color:#555;">
Original Order: {{ $originalOrder->order_number }}
</p>
...
<p style="font-size:11px;color:#9a6c4c;font-weight:bold;text-transform:uppercase;">Returned Item</p>
<p style="margin:0;font-weight:bold;">{{ $originalOrder->item_name }}</p>
@if($originalOrder->size)
<p style="margin:0;">Size: {{ $originalOrder->size }}</p>
@endif
</td>

<td width="50%" valign="top" style="width:50%;" class="two-column">
<p style="font-size:11px;color:#9a6c4c;font-weight:bold;text-transform:uppercase;">Exchanged For</p>
<p style="margin:0;font-weight:bold;">{{ $exchangeOrder->item_name }}</p>
@if($exchangeOrder->size)
<p style="margin:0;">Size: {{ $exchangeOrder->size }}</p>
@endif
</td>
...
<p style="margin:0;font-weight:bold;">{{ $exchangeOrder->item_name }}</p>
@if($exchangeOrder->size)
<p style="margin:2px 0;color:#9a6c4c;">Size: {{ $exchangeOrder->size }}</p>
@endif
<p style="margin:0;color:#9a6c4c;">Qty: {{ $exchangeOrder->quantity }}</p>
<p style="margin:0;color:#9a6c4c;">Status: {{ ucfirst(str_replace('_', ' ', $exchangeOrder->order_status)) }}</p>
</td>

<td valign="top" align="right" style="font-weight:bold;width:100px;text-align:right;">
₹{{ number_format($exchangeOrder->total_amount, 2) }}
</td>
</tr>
</table>

<p style="margin:20px 0;color:#9a6c4c;">
Your exchange order is being processed and will be shipped to the same address as your original order. You'll receive tracking information once it ships.
</p>
...
<p style="margin:0 0 10px;color:#9a6c4c;">We're here to help if you have any issues with your exchange order.</p>
