...
@section('content')
    <div style="display:flex;justify-content:flex-end;align-items:center;margin-bottom:24px;">
        <a href="{{ route('inventory.admin.returns-exchange.index') }}" class="btn-back-outline">
        ...
        <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;margin-bottom:16px;">
            ...
            <div style="border:1px solid #e5e7eb;border-radius:12px;padding:16px;">
                ...
                @if($returnRequest->order)
                    <p><a href="{{ route('inventory.admin.orders.show', $returnRequest->order) }}" style="color:#490d59;text-decoration:none;">View order details →</a></p>
                @else
        ...
        <div style="display:flex;gap:8px;flex-wrap:wrap;">
            @if($returnRequest->status === 'pending')
                <div style="display:flex; gap:8px;">
                    <form method="POST" action="{{ route('inventory.admin.returns-exchange.approve', $returnRequest) }}" style="display:flex;gap:8px;align-items:center;">
                        @csrf
                    ...
                    @if($returnRequest->type === 'return')
                        <form method="POST" action="{{ route('inventory.admin.returns-exchange.switch-type', $returnRequest) }}" onsubmit="return confirm('Are you sure you want to switch this request to an Exchange?');">
                            @csrf
                    ...
                    <form method="POST" action="{{ route('inventory.admin.returns-exchange.deny', $returnRequest) }}" onsubmit="return confirm('Are you sure you want to deny this request?');">
                        @csrf
            ...
            @if(in_array($returnRequest->status, ['approved', 'received_restocked', 'received_discarded']) && $returnRequest->type === 'return')
                @if($returnRequest->status === 'approved')
                    <form method="POST" action="{{ route('inventory.admin.returns-exchange.receive', $returnRequest) }}" style="display:flex;gap:8px;align-items:center;">
                        @csrf
                ...
                <form method="POST" action="{{ route('inventory.admin.returns-exchange.refund', $returnRequest) }}" onsubmit="return confirm('Are you sure you want to process a refund for this request? This will initiate a Razorpay refund.');">
                    @csrf
            ...
            @if($returnRequest->status === 'approved' && $returnRequest->type === 'exchange')
                <form method="POST" action="{{ route('inventory.admin.returns-exchange.generate', $returnRequest) }}" style="display:grid;gap:8px;max-width:400px;">
                    @csrf
