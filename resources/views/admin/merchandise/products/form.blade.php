...
@section('content')
    <div style="display:flex;justify-content:flex-end;align-items:center;margin-bottom:24px;">
        ...
        <a href="{{ route('admin.merchandise.products.index') }}" class="btn-back-outline">
            <i class="fas fa-arrow-left"></i> Back to catalog
        </a>
    </div>

    <form method="POST" action="{{ $isEdit ? route('admin.merchandise.products.update', $product) : route('admin.merchandise.products.store') }}" enctype="multipart/form-data">
        @csrf
        @if($isEdit)
            @method('PUT')
        @endif
                        ...
                        @if($product->media_images)
                            @foreach($product->media_images as $index => $img)
                                @php
                                    ...
                                    if(empty($finalImgUrl)) continue;
                                    
                                    $isVideo = preg_match('/\.(mp4|webm|ogg|mov|avi|wmv|flv|mkv|m3u8)(\?.*)?$/i', $finalImgUrl);
                                    $mediaUrl = Str::startsWith($finalImgUrl, 'http') ? $finalImgUrl : asset('storage/' . $finalImgUrl);
                                @endphp
