...
@section('title', 'Email Templates | The Skool Store')
@section('page_heading', 'Email Templates')
@section('page_subheading', 'Manage email templates for notifications')

@section('content')
    <div class="card" style="max-width:1100px;margin:auto;">
        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:24px;">
            <h3 style="margin:0;color:#111827;">Email Templates</h3>
                ...
                <div style="border:1px solid #e5e7eb;border-radius:16px;padding:20px;">
                    <div style="display:flex;justify-content:space-between;align-items:start;">
                        <div style="flex:1;">
                            ...
                            <p style="margin:0 0 4px;color:#6b7280;font-size:14px;"><strong>Subject:</strong> {{ $template->subject }}</p>
                        </div>
                        <div style="display:flex;gap:8px;">
                            ...
                            <form method="POST" action="{{ route('master.admin.settings.email-templates.destroy', $template) }}" style="margin:0;" onsubmit="return confirm('Are you sure?')">
                                @csrf
                                @method('DELETE')
        ...
        <div style="margin-top:24px;padding:20px;border:1px dashed #d0d5dd;border-radius:16px;background:#f9fafb;">
            ...
            <form method="POST" action="{{ route('master.admin.settings.email-templates.store') }}">
                @csrf
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px;">
                    ...
                    <label>
                        ...
                        <small style="color:#6b7280;font-size:12px;">Unique identifier (e.g., order_confirmation, order_shipped)</small>
                    </label>
                </div>
                ...
                <div style="margin-top:16px;">
                    <label>
                        <span>Text Body (optional)</span>
                        <textarea name="body_text" rows="6"></textarea>
                    </label>
                </div>
                <div style="margin-top:16px;">
    ...
    @foreach($templates as $template)
        <div id="editForm{{ $template->id }}" style="display:none;position:fixed;top:50%;left:50%;transform:translate(-50%,-50%);background:#fff;border:1px solid #e5e7eb;border-radius:16px;padding:24px;max-width:800px;width:90%;max-height:90vh;overflow-y:auto;z-index:1000;box-shadow:0 20px 60px rgba(0,0,0,0.3);">
            <h4 style="margin:0 0 20px;color:#111827;">Edit Email Template</h4>
            <form method="POST" action="{{ route('master.admin.settings.email-templates.update', $template) }}">
                @csrf
                @method('PUT')
                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(250px,1fr));gap:16px;">
                    <label>
                ...
                <div style="margin-top:16px;">
                    <label>
                        <span>HTML Body *</span>
                        <textarea name="body_html" rows="10" required>{{ $template->body_html }}</textarea>
                    </label>
                </div>
                <div style="margin-top:16px;">
                    <label>
                        <span>Text Body (optional)</span>
                        <textarea name="body_text" rows="6">{{ $template->body_text }}</textarea>
                    </label>
                </div>
                <div style="margin-top:16px;">
                    <label style="flex-direction:row;align-items:center;gap:8px;">
