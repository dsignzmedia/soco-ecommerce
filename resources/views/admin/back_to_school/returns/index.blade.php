...
@section('content')
    ...
    <div class="filters-card">
        <form method="GET" action="{{ route('admin.back_to_school.returns-exchange.index') }}" class="filter-form-grid">
            ...
            <a href="{{ route('admin.back_to_school.returns-exchange.index') }}" class="btn-reset">Reset</a>
        </form>
    </div>
                    ...
                    <tr>
                        ...
                        <td>
                            <div class="action-group">
                                <a href="{{ route('admin.back_to_school.returns-exchange.show', $req) }}" class="btn-action-sm btn-view">
                                    <i class="fas fa-eye" style="margin-right:4px;"></i> View
                                </a>

                                @if($req->status === 'pending')
                                    <form method="POST" action="{{ route('admin.back_to_school.returns-exchange.approve', $req) }}">
                                        @csrf
                                        <button type="submit" class="btn-action-sm btn-approve" title="Approve">Approve</button>
                                    </form>
                                    <form method="POST" action="{{ route('admin.back_to_school.returns-exchange.deny', $req) }}">
                                        @csrf
                                ...
                                @if(in_array($req->status, ['approved', 'received_restocked', 'received_discarded']))
                                    @if($req->type === 'return')
                                        @if($req->status === 'approved')
                                            <form method="POST" action="{{ route('admin.back_to_school.returns-exchange.receive', $req) }}" style="display:flex;gap:6px;">
                                                @csrf
                                        ...
                                        <form method="POST" action="{{ route('admin.back_to_school.returns-exchange.refund', $req) }}" onsubmit="return confirm('Are you sure you want to process a refund for this request? This will initiate a Razorpay refund.');">
                                            @csrf
                                    ...
                                    @if($req->type === 'exchange')
                                        <a href="{{ route('admin.back_to_school.returns-exchange.show', $req) }}" class="btn-action-sm btn-blue" style="border: 2px solid #0284c7;">
