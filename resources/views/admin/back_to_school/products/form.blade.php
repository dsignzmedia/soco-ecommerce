            ...
            <div style="display:flex;flex-direction:column;gap:24px;">
                ...
                <div class="card">
                    ...
                    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:16px;">
                        ...
                        {{-- Delivery Price Removed per user request --}}
                        {{-- 
                        <label>
                            <span>Delivery Price</span>
                            <input type="number" name="delivery_price" min="0" step="0.01" value="{{ old('delivery_price', $product->delivery_price) }}">
                        </label>
                        --}}
                        <label>
                            <span>Tax (%)</span>
                    ...
                    <div style="display:flex;flex-direction:column;gap:16px;">
                        ...
                        <label>
                            ...
                            <select name="category">
                                ...
                                <option value="School-Day Essentials" @selected(old('category', $product->category) === 'School-Day Essentials')>School-Day Essentials</option>
                                @foreach($categories as $key => $label)
                                    @if($key !== 'School-Day Essentials')
...
@push('scripts')
    ...
    <style>
        .media-item { position: relative; width: 120px; height: 120px; border: 2px solid #ddd; border-radius: 8px; overflow: hidden; cursor: grab; transition: all 0.3s ease; user-select: none; background: white; }
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            if(addBtn) {
                addBtn.addEventListener('click', function() {
                    const index = new Date().getTime(); // Unique ID to avoid collisions
